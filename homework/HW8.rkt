;; The first three lines of this file were inserted by DrRacket. They record metadata
;; about the language level of this file in a form that our tools can easily process.
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname HW8) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))

(require 2htdp/image)
(require 2htdp/universe)

;; Usual Instructions:
;; 1. Do not create, modify or delete any line that begins with ";;!". These are
;;    markers that we use to segment your file into parts to facilitate grading.
;; 2. You must follow the _design recipe_ for every problem. In particular,
;;    every function you define must have at least three check-expects (and
;;    more if needed).
;; 3. You must follow the Style Guide:
;;    https://pages.github.khoury.northeastern.edu/2500/2024F/style.html
;; 4. You must submit working code. In DrRacket, ensure you get no errors
;;    when you click Run. After you submit on Gradescope, you'll get instant
;;    feedback on whether or not Gradescope can run your code, and your code must
;;    run on Gradescope to receive credit from the autograder.
;; 5. On some problems, you can get automated feedback on your in-progress work
;;    from FeedBot, a system developed by the course staff. When you submit your
;;    assignment, you will see a link to the FeedBot report along with the autograder
;;    feedback. Only a certain number of submissions will get this, and submissions
;;    close together will not receive the feedback.


;; In this assignment, you are going to build upon the work you did in HW6 to
;; build an interactive game that simulates atmospheric CO2 into the future,
;; based on converting commuters to public transit and planting eelgrass. You
;; will do this by extending your existing neighborhood data definitions to
;; store total emissions in each neighborhood, which will then be modified by
;; converting commuters & planting eelgrass.

;; Eelgrass is part of the Blue Carbon coastal ecosystem that sequester significant
;; amounts of CO2, and there is current research on how effective it is in Massachusetts.
;; See, e.g., https://oceanservice.noaa.gov/ecosystems/coastal-blue-carbon/
;; and https://seagrant.mit.edu/2023/10/19/mapping-blue-carbon-new-report-highlights-coastal-ecosystem-benefits/

;; An estimate from the latter is that, in Massachusetts, 218,222 acres sequester
;; a collective 7.5 million metric tonnes of carbon, or around 34 tonnes per
;; acre.

;;! Problem 1

;;! Part A

;; In this problem, you will refine and add to your neighborhood definitions from
;; previous assignment(s).

;; 1. First, copy your existing BostonCommuters data definition from HW6, along with all
;;    the definitions it relies upon.

;; 2. After copying your definitions, please fix any issues identified in the
;;    feedback you got on your HW6 submission.

;; 3. For reference, you should have the following data definitions:
;;    - Neighborhood: Represents one of 5 neighborhoods in Boston
;;    - NeighborhoodCommuters: A data definition with three fields:
;;      - the Neighborhood
;;      - the number of public transit commuters in that Neighborhood
;;      - the total number of commuters in that Neighborhood
;;    - BostonCommuters: Represents a collection (list) of NeighborhoodCommuters.

;; 4. Now for this problem, modify the NeighborhoodCommuters data definition to
;;    include a field that holds the total carbon emitted, **IN METRIC TONNES**,
;;    for that Neighborhood. This means the struct should now have four fields.

;;!! IMPORTANT: Write your response BELOW this line:

;; A Neighborhood is one of:
;; - "East Boston"
;; - "West Roxbury"
;; - "Hyde Park"
;; - "Brighton"
;; - "Dorchester"
;; Interpretation: A neighborhood is one of 23 named neighborhoods in Boston
;; Examples: "East Boston", "West Roxbury", "Hyde Park"

(define EAST-BOSTON "East Boston")
(define WEST-ROXBURY "West Roxbury")
(define HYDE-PARK "Hyde Park")
(define BRIGHTON "Brighton")
(define DORCHESTER "Dorchester")

;; neighborhood-template : Neighborhood -> ?
(define (neighborhood-template n)
  (cond
    [(string=? n EAST-BOSTON) ...]
    [(string=? n WEST-ROXBURY) ...]
    [(string=? n HYDE-PARK) ...]
    [(string=? n BRIGHTON) ...]
    [(string=? n DORCHESTER) ...]))

;; A NeighborhoodCommuters is a (make-neighborhood-commuters Neighborhood Number Number Number)
(define-struct neighborhood-commuters [name public-transit commuters carbon-emitted])
;; Represents a NeighborhoodCommuters where:
;; - name is the Neighborhood
;; - public transit is the number of commuters who use public transit in the neighborhood
;; - commuters is the total number of commuters in the neighborhood
;; - carbon-emitted is the total carbon emissions in metric tonnes

;; make-neighborhood-commuters : Neighborhood Number Number Number -> NeighborhoodCommuters
;; neighborhood-commuters? : Any -> Boolean
;; neighborhood-commuters-name : NeighborhoodCommuters -> Neighborhood
;; neighborhood-commuters-public-transit : NeighborhoodCommuters -> Number
;; neighborhood-commuters-commuters : NeighborhoodCommuters -> Number
;; neighborhood-commuters-carbon-emitted : NeighborhoodCommuters -> Number

(define EAST-BOSTON-NC (make-neighborhood-commuters EAST-BOSTON 14110 27151 0))
(define WEST-ROXBURY-NC (make-neighborhood-commuters WEST-ROXBURY 2919 18483 0))
(define HYDE-PARK-NC (make-neighborhood-commuters HYDE-PARK 4548 19365 0))
(define BRIGHTON-NC (make-neighborhood-commuters BRIGHTON 11007 33229 0))
(define DORCHESTER-NC (make-neighborhood-commuters DORCHESTER 22294 64311 0))

;; NeighborhoodCommuters -> ?
(define (neighborhood-commuters-temp neighborhood-commuters)
    (...(neighborhood-template (neighborhood-commuters-name neighborhood-commuters))
        ...(neighborhood-commuters-public-transit neighborhood-commuters)
        ...(neighborhood-commuters-commuters neighborhood-commuters)
        ...(neighborhood-commuters-carbon-emitted neighborhood-commuters)))

;; A BostonCommuters (BC) is one of:
;; - '()
;; - (cons NeighborhoodCommuters BC)
;; Interpretation: represents multiple neighborhoods within Boston

(define BC-1 '())
(define BC-2 (cons EAST-BOSTON-NC BC-1))
(define BC-3 (cons DORCHESTER-NC BC-2))
(define BC-4 (cons HYDE-PARK-NC (cons BRIGHTON-NC (cons WEST-ROXBURY-NC (cons DORCHESTER-NC BC-2)))))

;; bc-temp : BC -> ?
(define (bc-temp bc) 
  (...(cond [(empty? bc) ...] 
            [(cons? bc)
             ...(neighborhood-commuters-temp (first bc)) ...
             ...(bc-temp (rest bc)) ...])))

;;! Part B

;; Define an example of your data definition, that is suitable for showing your graphics, as:
;; the constant `BC-EX`.

;;!! IMPORTANT: Write your response BELOW this line:

(define BC-EX BC-4)

;;! Part C

;; In HW6, you wrote a function, `simulate-emissions`, that calculated the
;; emissions caused by non-public transit commuters a given number of years into
;; the future. If it did not assume that everyone who does not use public
;; transit drives a car, we'd like you to fix that calculation for the sake of
;; the game -- while this certainly rules out other important commuting
;; categories (biking, walking), this will more easily allow the game to work.

;; We'd like you to, based on your existing function, design a new function
;; `add-emissions-one-year` that takes a NeighborhoodCommuters and returns an
;; updated version, where **one year** of emissions have been added to the new
;; field of total carbon emitted, based on the existing number of commuters who
;; drive (which should be all who do not use public transit). You can either use
;; your existing helper (passing 1 as the number of years) or you can use the
;; existing calculation when designing this new function.

;; Ensure the units of your emissions are in METRIC TONNES. If you used grams,
;; divide by 1000000.

;;!! IMPORTANT: Write your response BELOW this line:

;; simulate-emissions : NeighborhoodCommuters Number -> Number
;; computes the total CO2 emissions from people that are commuting by car in a given neighborhood
;; over a specified number of years; 40 miles per commute two ways, 400 grams of C02 per mile, 365
;; days per year, divided by 1000000 to return metric tonnes
(define (simulate-emissions nc years)
  (/ (* (- (neighborhood-commuters-commuters nc)
        (neighborhood-commuters-public-transit nc))
     40      ;; miles per commute two ways
     400     ;; grams of CO2 per mile
     365     ;; days per year
     years) 1000000))

;; Test Cases
(check-expect (simulate-emissions EAST-BOSTON-NC 10) 761594.400000) 
(check-expect (simulate-emissions WEST-ROXBURY-NC 25) 2272344) 
(check-expect (simulate-emissions DORCHESTER-NC 40) 9815171.2)

;; add-emissions-one-year : NeighborhoodCommuters -> NeighborhoodCommuters
;; takes a NeighborhoodCommuters and returns an updated version where one year of emissions have
;; been added to the total carbon emissions based on the existing number of commuters who driver
(define (add-emissions-one-year nc)
  (make-neighborhood-commuters (neighborhood-commuters-name nc)
                               (neighborhood-commuters-public-transit nc)
                               (neighborhood-commuters-commuters nc)
                               (+ (neighborhood-commuters-carbon-emitted nc)
                                  (simulate-emissions nc 1))))

;; Test Cases
(check-expect (add-emissions-one-year EAST-BOSTON-NC)
              (make-neighborhood-commuters (neighborhood-commuters-name EAST-BOSTON-NC)
                               (neighborhood-commuters-public-transit EAST-BOSTON-NC)
                               (neighborhood-commuters-commuters EAST-BOSTON-NC)
                               (+ (neighborhood-commuters-carbon-emitted EAST-BOSTON-NC)
                                  (simulate-emissions EAST-BOSTON-NC 1))))
(check-expect (add-emissions-one-year WEST-ROXBURY-NC)
              (make-neighborhood-commuters (neighborhood-commuters-name WEST-ROXBURY-NC)
                               (neighborhood-commuters-public-transit WEST-ROXBURY-NC)
                               (neighborhood-commuters-commuters WEST-ROXBURY-NC)
                               (+ (neighborhood-commuters-carbon-emitted WEST-ROXBURY-NC)
                                  (simulate-emissions WEST-ROXBURY-NC 1))))
(check-expect (add-emissions-one-year DORCHESTER-NC)
              (make-neighborhood-commuters (neighborhood-commuters-name DORCHESTER-NC)
                               (neighborhood-commuters-public-transit DORCHESTER-NC)
                               (neighborhood-commuters-commuters DORCHESTER-NC)
                               (+ (neighborhood-commuters-carbon-emitted DORCHESTER-NC)
                                  (simulate-emissions DORCHESTER-NC 1))))

;;! Part D

;; In order to draw the game, we need to copy your `draw-boston-emissions`
;; function from HW6, and all its helpers. If you received feedback in HW6,
;; (e.g., about how the neighborhoods were positioned) please apply corrections
;; first.
;;
;; However -- in order to be able to be passed to `big-bang`, the only argument
;; to it must be `BostonCommuters`, so remove the `scale` & `years` arguments.
;; Instead, pass a fixed scale to the helpers, and render at the given moment.
;; The other change you should make is in your function to draw a single
;; neighborhood: since the carbon emissions are now stored in a field, use that
;; field to determine the color, rather than calling `simulate-emissions`.
;;
;; Ensure that your coloring makes it clear how carbon is increasing in
;; neighborhoods: we would suggest some sort of rainbow, perhaps ending with a
;; lighter color when total CO2 in a neighborhood reaches 0 (which will correspond
;; to winning the game!).

;;!! IMPORTANT: Write your response BELOW this line:

;; get-color : Number -> Color
;; Takes emissions and returns a shade of red, with light color for low emissions and darker for
;; high emissions.
(define (get-color emissions)
  (make-color (max 10 (- 255 (min 255 (floor (/ emissions 40000))))) 10 10))

;; Test Cases
(check-expect (get-color 10000) (make-color 255 10 10))
(check-expect (get-color 80000) (make-color 253 10 10))
(check-expect (get-color 1000000) (make-color 230 10 10))

;; draw-neighborhood : NeighborhoodCommuters -> Image
;; returns an image of the neighborhood given the neighborhood-commuters
(define (draw-neighborhood nc)
  (cond
    [(string=? (neighborhood-commuters-name nc) EAST-BOSTON)
     (place-image (rectangle 50 50 "solid" (get-color (neighborhood-commuters-carbon-emitted nc)))
                  175
                  25
                  (rectangle 200 250 "solid" "transparent"))]
    [(string=? (neighborhood-commuters-name nc) WEST-ROXBURY)
     (place-image (rectangle 55 80 "solid" (get-color (neighborhood-commuters-carbon-emitted nc)))
                  20
                  180
                  (rectangle 200 250 "solid" "transparent"))]
    [(string=? (neighborhood-commuters-name nc) HYDE-PARK)
     (place-image (rectangle 45 70 "solid" (get-color (neighborhood-commuters-carbon-emitted nc)))
                  70
                  225
                  (rectangle 200 250 "solid" "transparent"))]
    [(string=? (neighborhood-commuters-name nc) BRIGHTON)
     (place-image (rectangle 40 40 "solid" (get-color (neighborhood-commuters-carbon-emitted nc)))
                  35
                  45
                  (rectangle 200 250 "solid" "transparent"))]
    [(string=? (neighborhood-commuters-name nc) DORCHESTER)
     (place-image (rectangle 60 70 "solid" (get-color (neighborhood-commuters-carbon-emitted nc)))
                  150
                  130
                  (rectangle 200 250 "solid" "transparent"))]))

;; Test Cases
(check-expect 
  (draw-neighborhood EAST-BOSTON-NC)
  (place-image (rectangle 50 50 "solid"
                          (get-color (neighborhood-commuters-carbon-emitted EAST-BOSTON-NC)))
               175
               25
               (rectangle 200 250 "solid" "transparent")))
(check-expect 
  (draw-neighborhood WEST-ROXBURY-NC)
  (place-image (rectangle 55 80 "solid"
                          (get-color (neighborhood-commuters-carbon-emitted WEST-ROXBURY-NC)))
               20
               180
               (rectangle 200 250 "solid" "transparent")))
(check-expect 
  (draw-neighborhood BRIGHTON-NC)
  (place-image (rectangle 40 40 "solid"
                          (get-color (neighborhood-commuters-carbon-emitted BRIGHTON-NC)))
               35
               45
               (rectangle 200 250 "solid" "transparent")))

;; draw-boston-emissions : BostonCommuters -> Image
;; takes a boston-commuters and draws only the neighborhoods provided, color-coded by the amount of 
;; CO2 emissions, positioning them based on a rough geographic layout
(define (draw-boston-emissions bc)
  (cond
    [(empty? bc) (rectangle 200 250 "solid" "white")] 
    [else
     (overlay
      (draw-neighborhood (first bc))
      (draw-boston-emissions (rest bc)))]))

;; Test Cases
(check-expect (draw-boston-emissions BC-1) (rectangle 200 250 "solid" "white"))
(check-expect (draw-boston-emissions BC-2)
              (overlay (draw-neighborhood EAST-BOSTON-NC)
                       (rectangle 200 250 "solid" "white")))
(check-expect (draw-boston-emissions BC-3)
              (overlay (draw-neighborhood DORCHESTER-NC)
                       (overlay (draw-neighborhood EAST-BOSTON-NC)
                                (rectangle 200 250 "solid" "white"))))

;;! Part E

;; Now that we can draw, we can design a function `tick-boston-emissions`, which
;; takes a `BostonCommuters` and returns a new one, where one year has
;; passed. Use the helper you defined in Part C, and consider if there are any
;; useful list abstractions you can use.

;;!! IMPORTANT: Write your response BELOW this line:

;; tick-boston-emissions : BostonCommuters -> BostonCommuters
;; takes a BostonCommuters and returns a new one, where one year has passed
(define (tick-boston-emissions bc)
  (map add-emissions-one-year bc))

;; Test Cases
(check-expect (tick-boston-emissions BC-1) '())
(check-expect (tick-boston-emissions BC-2) (list (add-emissions-one-year EAST-BOSTON-NC)))
(check-expect (tick-boston-emissions BC-3) (list (add-emissions-one-year DORCHESTER-NC)
                                                 (add-emissions-one-year EAST-BOSTON-NC)))

;;! Part F

;; While with tick & draw, we could run the simulation, to make this a game, we
;; want to add two elements that allow the player to **decrease** CO2.
;;
;; First, we want to make it so that if the user presses any key, it causes a
;; constant number of acres of eelgrass to be planted (the constant is provided;
;; you can tweak it to alter the gameplay, though the default should work).
;;
;; Design a function `key-boston-emissions` that does exactly that. Note that,
;; based on the intro, one acre eelgrass results in 34 metric tonnes of carbon
;; removed; we will simplify and assume the effect is split evenly across each
;; of the 23 neighborhoods, so you should **remove 1.5 metric tonnes of carbon
;; per acre per neighborhood per keypress**.

(define ACRES-PLANTED-PER-PRESS 500)

;;!! IMPORTANT: Write your response BELOW this line:

;; CO2 removal per neighborhood per keypress, based on 500 acres planted
(define CO2-REMOVAL-PER-NEIGHBORHOOD (* ACRES-PLANTED-PER-PRESS 1.5))

;; reduce-carbon : NeighborhoodCommuters -> NeighborhoodCommuters
;; decreases the carbon emitted by a fixed amount per 500 acres of planted eelgrass
(define (reduce-carbon nc)
  (make-neighborhood-commuters 
   (neighborhood-commuters-name nc)
   (neighborhood-commuters-public-transit nc)
   (neighborhood-commuters-commuters nc)
   (- (neighborhood-commuters-carbon-emitted nc) CO2-REMOVAL-PER-NEIGHBORHOOD)))

;; Test Cases
(check-expect (reduce-carbon (make-neighborhood-commuters
                              (neighborhood-commuters-name EAST-BOSTON-NC)
                              (neighborhood-commuters-public-transit EAST-BOSTON-NC)
                              (neighborhood-commuters-commuters EAST-BOSTON-NC)
                              750))
              (make-neighborhood-commuters
               (neighborhood-commuters-name EAST-BOSTON-NC)
               (neighborhood-commuters-public-transit EAST-BOSTON-NC)
               (neighborhood-commuters-commuters EAST-BOSTON-NC)
               0))
(check-expect (reduce-carbon (make-neighborhood-commuters
                              (neighborhood-commuters-name WEST-ROXBURY-NC)
                              (neighborhood-commuters-public-transit WEST-ROXBURY-NC)
                              (neighborhood-commuters-commuters WEST-ROXBURY-NC)
                              1500))
              (make-neighborhood-commuters
               (neighborhood-commuters-name WEST-ROXBURY-NC)
               (neighborhood-commuters-public-transit WEST-ROXBURY-NC)
               (neighborhood-commuters-commuters WEST-ROXBURY-NC)
               750))
(check-expect (reduce-carbon (make-neighborhood-commuters
                              (neighborhood-commuters-name HYDE-PARK-NC)
                              (neighborhood-commuters-public-transit HYDE-PARK-NC)
                              (neighborhood-commuters-commuters HYDE-PARK-NC)
                              2500))
              (make-neighborhood-commuters
               (neighborhood-commuters-name HYDE-PARK-NC)
               (neighborhood-commuters-public-transit HYDE-PARK-NC)
               (neighborhood-commuters-commuters HYDE-PARK-NC)
               1750))

;; key-boston-emissions : BostonCommuters KeyEvent -> BostonCommuters
;; takes a BostonCommuters and key event and returns a new one with reduced CO2 in each neighborhood
;; after a key press
(define (key-boston-emissions bc ke)
  (if (key-event? ke) (map reduce-carbon bc) bc))

;; Test Cases
(check-expect (key-boston-emissions BC-1 "down") (map reduce-carbon BC-1))
(check-expect (key-boston-emissions BC-2 "up") (map reduce-carbon BC-2))
(check-expect (key-boston-emissions BC-3 "left") (map reduce-carbon BC-3))

;;! Part G

;; We want there to be a second mechanism for the game: clicking the mouse
;; should convert a constant number of commuters from driving to using public
;; transit in each neighborhood (provided below), assuming there are any left.
;;
;; Design this function, which should be called `mouse-boston-emissions`, as
;; well.

(define CONVERTERS-PER-CLICK 300)

;;!! IMPORTANT: Write your response BELOW this line:

;; commuters-to-public-transit : NeighborhoodCommuters -> NeighborhoodCommuters
;; converts 300 commuters from driving to public transit
(define (commuters-to-public-transit nc)
  (make-neighborhood-commuters 
   (neighborhood-commuters-name nc)
   (+ (neighborhood-commuters-public-transit nc)
      (min CONVERTERS-PER-CLICK 
           (- (neighborhood-commuters-commuters nc)
              (neighborhood-commuters-public-transit nc))))
   (neighborhood-commuters-commuters nc)
   (neighborhood-commuters-carbon-emitted nc)))

;; Test Cases
(check-expect (commuters-to-public-transit EAST-BOSTON-NC)
              (make-neighborhood-commuters
               (neighborhood-commuters-name EAST-BOSTON-NC)
               (+ 300 (neighborhood-commuters-public-transit EAST-BOSTON-NC))
               (neighborhood-commuters-commuters EAST-BOSTON-NC)
               (neighborhood-commuters-carbon-emitted EAST-BOSTON-NC)))
(check-expect (commuters-to-public-transit WEST-ROXBURY-NC)
              (make-neighborhood-commuters
               (neighborhood-commuters-name WEST-ROXBURY-NC)
               (+ 300 (neighborhood-commuters-public-transit WEST-ROXBURY-NC))
               (neighborhood-commuters-commuters WEST-ROXBURY-NC)
               (neighborhood-commuters-carbon-emitted WEST-ROXBURY-NC)))
(check-expect (commuters-to-public-transit DORCHESTER-NC)
              (make-neighborhood-commuters
               (neighborhood-commuters-name DORCHESTER-NC)
               (+ 300 (neighborhood-commuters-public-transit DORCHESTER-NC))
               (neighborhood-commuters-commuters DORCHESTER-NC)
               (neighborhood-commuters-carbon-emitted DORCHESTER-NC)))

;; mouse-boston-emissions : BostonCommuters Number Number MouseEvent -> BostonCommuters
;; applies the conversion of commuters to public transit in each neighborhood
;; for each mouse click button down
(define (mouse-boston-emissions bc x y me)
  (if (mouse=? me "button-down") (map commuters-to-public-transit bc) bc))

;; Test Cases
(check-expect (mouse-boston-emissions BC-1 0 0 "button-down") (map commuters-to-public-transit BC-1))
(check-expect (mouse-boston-emissions BC-2 0 0 "button-down") (map commuters-to-public-transit BC-2))
(check-expect (mouse-boston-emissions BC-3 0 0 "button-up") BC-3)

;;! Part H

;; Also, we'd like to be able to win the game! Define a helper function
;; `win?` that, given a BostonCommuters, returns #true if any of the
;; neighborhoods have reached 0 carbon.

;;!! IMPORTANT: Write your response BELOW this line:

;; win? : BostonCommuters -> Boolean
;; returns true if any neighborhood in BostonCommuters has reached 0 carbon emissions
(define (win? bc)
  (ormap (lambda (nc) (<= (neighborhood-commuters-carbon-emitted nc) 0)) bc))

;; Test Cases
(check-expect (win? BC-1) #false)
(check-expect (win? BC-2) #true)
(check-expect (win? BC-3) #true)

;;! Part I

;; The final helper we'd like to define is a function, `draw-win`, to display a "You win!"
;; message. It should take the game state (BostonCommuters) and return an image
;; that represents a winning game. We'd suggest text overlaying the existing map.

;;!! IMPORTANT: Write your response BELOW this line:

;; draw-win : BostonCommuters -> Image
;; displays a "You win!" message overlaying the current map
(define (draw-win bc)
  (overlay (text "You win!" 40 "green") 
           (draw-boston-emissions bc)))

;; Test Cases
(check-expect (draw-win BC-1) (overlay (text "You win!" 40 "green") 
           (draw-boston-emissions BC-1)))
(check-expect (draw-win BC-2) (overlay (text "You win!" 40 "green") 
           (draw-boston-emissions BC-2)))
(check-expect (draw-win BC-3) (overlay (text "You win!" 40 "green") 
           (draw-boston-emissions BC-3)))

;;! Part J

;; Now design a function `play-game`, that takes an initial `BostonCommuters`
;; and invokes `big-bang` with the handlers designed in part D, E, F, G, H, I.
;; When run, it should allow you to try to keep ahead of the carbon by either
;; planting eelgrass or convincing people to take public transit, and if they
;; win (get one neighborhood to 0), it should display the winning message (if
;; you haven't already, consult the documentation for the `stop-when` clause of
;; big-bang). To make the game playable, slow it down so that each year
;; (represented by one tick) occurs per second (i.e. `[on-tick
;; tick-boston-emissions 1]`).
;;
;; In order to not have your initial state immediately be a "win" (as no carbon
;; will be recorded as having been emitted yet), we suggest that rather than
;; passing the argument as the initial state to `big-bang`, instead pass
;; `(tick-boston-emissions bc)` -- i.e., begin the game after 1 year.

;;!! IMPORTANT: Write your response BELOW this line:

;; play-game : BostonCommuters -> BostonCommuters
;; starts the game
(define (play-game bc)
  (big-bang (tick-boston-emissions bc)
    [to-draw draw-boston-emissions]
    [on-tick tick-boston-emissions 1]
    [on-key key-boston-emissions]
    [on-mouse mouse-boston-emissions]
    [stop-when win? draw-win]))