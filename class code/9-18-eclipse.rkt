...
;; next-moon2 : Position -> Position
;; produce the next moon at the next tick that is shifted by the a random number in [-1. 1] in both x and y
;; (check-expect (next-moon2 POSN-4)
;;              (make-posn (sub1 (random 3)) (sub1 (random 3))))
(define (next-moon2 p)
  (make-posn (+ (sub1 (random 3)) (posn-x p)
                (sub1 (random 3)) (posn-x p))))
