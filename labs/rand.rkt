...
(define (lost? tagid loo loe)
  (local [
          (define (encounters-with-tag tagid loe) (filter (lambda (e) (string=? tagid (encounter-tagid e))) loe))
          (define (tag-owner tagid loo) (first (filter (lambda (o) (string=? tagid (owner-tagid o))) loo)))
          (define (encountered-by-owner? tagid tag-owner encounters-with-tag) (> (length (filter (lambda (e) (string=? (owner-phoneid tag-owner) (encounter-phoneid e))) encounters-with-tag)) 0))
          ]
    (not (encountered-by-owner? tagid (tag-owner tagid loo) (encounters-with-tag tagid loe)))))

(check-expect (lost? "t1" 
                    (list (make-owner "p1" "t1") 
                          (make-owner "p1" "t2") 
                          (make-owner "p2" "t3"))
                    (list (make-encounter "p2" "t1" 10 (make-posn 0 0)) 
                          (make-encounter "p3" "t1" 20 (make-posn 100 100)))) #true)

 (check-expect (lost? "t2" 
                    (list (make-owner "p1" "t1") 
                          (make-owner "p1" "t2") 
                          (make-owner "p2" "t3"))
                    (list (make-encounter "p1" "t2" 10 (make-posn 0 0)) 
                          (make-encounter "p2" "t2" 20 (make-posn 100 100))))  #false)


(check-expect (lost? "t3" 
                    (list (make-owner "p1" "t1") 
                          (make-owner "p1" "t2") 
                          (make-owner "p2" "t3"))
                    (list (make-encounter "p2" "t1" 10 (make-posn 0 0)) 
                          (make-encounter "p3" "t2" 20 (make-posn 100 100))))   #false)
