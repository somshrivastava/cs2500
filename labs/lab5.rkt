...
;; draw-mouse-game : MouseGame -> Image
;; draws the current state of the MouseGame
(define (draw-mouse-game mg)
  (cond [(and (string? mg) (string=? mg "waiting")) (place-image (text "Waiting" 24 "black") (/ DIM 2) (/ DIM 2) BACKGROUND)]
        [(posn? mg) (draw-arrows mg)]
        [(and (string? mg) (string=? mg "out-of-bounds")) (place-image (text "Out of Bounds" 24 "black") (/ DIM 2) (/ DIM 2) BACKGROUND)]))

;; Test Cases
(check-expect (draw-mouse-game MGW) (place-image (text "Waiting" 24 "black") (/ DIM 2) (/ DIM 2) BACKGROUND))
(check-expect (draw-mouse-game MGP) (draw-arrows MGP))
(check-expect (draw-mouse-game MGO) (place-image (text "Out of Bounds" 24 "black") (/ DIM 2) (/ DIM 2) BACKGROUND))
...
;; handle-mouse : MouseGame Number Number MouseEvent -> MouseGame
;; takes the current MouseGame state and returns an image with the changed state
(define (handle-mouse mg x y mouse-event)
  (cond [(and (string? mg) (and (string=? mg "waiting") (mouse=? mouse-event "button-down"))) (make-posn x y)]
        [(and (posn? mg) (mouse=? mouse-event "move")) (make-posn x y)]
        [(and (posn? mg) (mouse=? mouse-event "leave")) "out-of-bounds"]
        [(and (string? mg) (and (string=? mg "out-of-bounds") (mouse=? mouse-event "button-down"))) (make-posn x y)]
        [else mg]))

;; Test Cases
(check-expect (handle-mouse MGW 50 50 "button-up") (make-posn 50 50))
(check-expect (handle-mouse MGP 50 50 "move") (make-posn 50 50))
(check-expect (handle-mouse MGO 50 50 "leave") "out-of-bounds")
...
;; mouse-game : MouseGame -> MouseGame
;; takes an initial game state and runs "big-bang"
(define (mouse-game mg)
  (big-bang mg
